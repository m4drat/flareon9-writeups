    ...
    with open('check_flag.js', 'rb') as src:
        for line in src:
            ...
            if patch_next_bigint:
                ...
                if b'Math.random' in line:
                    patched_line = bytes(f'console.log("T:{condition_num.decode("ascii")}");Math.floor(Math.random())'.encode('ascii'))
                else:
                    patched_line = bytes(f'console.log("T:{condition_num.decode("ascii")}");'.encode('ascii'))
            ...
            if re.match(br'\s+if \([0-9]+\)', line):
                patch_next_int = True

            if re.match(br'\s+if \([0-9]+n\)', line):
                patch_next_bigint = True
                condition_num = re.match(br'\s+if \(([0-9]+n)\)', line)[1]
